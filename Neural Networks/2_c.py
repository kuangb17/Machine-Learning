...
class Network:
    ...
    def parameter_ini(self):
        self.w_hidden1 = np.zeros((self.hidden1_nodes - 1, self.input_nodes), dtype=float)
        self.w_hidden2 = np.zeros((self.hidden2_nodes - 1, self.hidden1_nodes), dtype=float)
        self.w_output = np.zeros((self.output_nodes, self.hidden2_nodes), dtype=float)
